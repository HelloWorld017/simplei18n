...
const stringifyConsecutive = <T>() =>
  map<T[], (T | string)[]>(output =>
    output.reduce<(T | string)[]>((prev, curr) => {
      const lastItem = prev.length >= 1 ? prev[prev.length - 1] : undefined;
...
const interpolation: Parjser<I18nAtomInterpolation> = anyChar().pipe(
  manyBetween(string('{'), string('}')),
  map(output => [I18nAtomKind.Interpolation, output.join('')]),
);

const escape: Parjser<string> = string('\\').pipe(
  qthen(string('\\').pipe(or(string('{')), or(string('<')))),
);

const tagName: Parjser<string> = letter().pipe(or(digit()));
const tag: Parjser<I18nAtomTag> = string('<')
  .pipe(
    qthen(
      tagName.pipe(
        many(),
  ...
  .pipe(
    thenPick(({ name, closes }) => {
      ...
      return atom.pipe(
        ...
        map(output => [I18nAtomKind.Tag, name, output]),
      );
    }),
  );

atom.init(escape.pipe(or(tag), or(interpolation), or(anyChar())));

const program: Parjser<I18nAtom[]> = atom.pipe(many(), stringifyConsecutive<I18nAtom>());

const parseAllI18n = <TKey extends string>(
  i18nData: Record<TKey, string>,
): Record<TKey, I18nAtom[]> =>
  Object.fromEntries(
    Object.entries(i18nData).map(([key, value]) => {
...
export const parse = <TKey extends string>(text: string): Record<TKey, I18nAtom[]> => {
  const i18nData = parseYaml(text) as Record<TKey, string>;
